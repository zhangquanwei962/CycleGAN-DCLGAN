...
class DCLModel(BaseModel):
    ...
    def calculate_NCE_loss1(self, src, tgt):
        ...
        feat_q = self.netG_B(tgt, self.nce_layers, encode_only=True)# falseB
        feat_k = self.netG_A(src, self.nce_layers, encode_only=True) # trueA falseB 
        ...
        total_nce_loss = 0.0
        for f_q, f_k, crit, _ in zip(feat_q_pool, feat_k_pool, self.criterionNCE, self.nce_layers):  # nce_layer
            loss = crit(f_q, f_k)
